...
class Ui_MainWindow(object):
    ...
    def denoise(self):
        ...
        self.showImage(self.lbl_input_image, self.output)
    ...
    def createKernel(self):
        ...
        self.row = self.spb_x.value()
        self.column = self.spb_y.value()
        value = flat[len(flat) // 2]/(self.row*self.column)
        # self.kernel_size = np.ones((self.spb_x.value(), self.spb_y.value()), np.float32) / (self.spb_x.value()*self.spb_y.value())
        self.kernel_size = np.full((self.row, self.column), value)
